...
TEST_CASE("ColorUtils::generate_rough_matte uses picked chromaticity and smooth boundaries",
          "[unit][color]") {
    ImageBuffer rgb_buf(4, 1, 3);
    Image rgb = rgb_buf.view();
    rgb(0, 0, 0) = 0.0F;
    rgb(0, 0, 1) = 0.0F;
    rgb(0, 0, 2) = 1.0F;
    ...
    ImageBuffer hint_buf(4, 1, 1);
    Image hint = hint_buf.view();
    ColorUtils::generate_rough_matte(rgb, hint, {0.0F, 0.0F, 1.0F}, 0.08F, 0.12F);

    CHECK(hint(0, 0, 0) == Catch::Approx(0.0F));
    CHECK(hint(0, 1, 0) == Catch::Approx(0.0F));
    CHECK(hint(0, 2, 0) == Catch::Approx(1.0F));
    CHECK(hint(0, 3, 0) == Catch::Approx(1.0F));
}

TEST_CASE("ColorUtils::generate_rough_matte applies smoothstep between tolerance and softness",
          "[unit][color]") {
    ImageBuffer rgb_buf(1, 1, 3);
    Image rgb = rgb_buf.view();
    rgb(0, 0, 0) = 0.15F;
    ...
    ImageBuffer hint_buf(1, 1, 1);
    Image hint = hint_buf.view();
    ColorUtils::generate_rough_matte(rgb, hint, {0.0F, 1.0F, 0.0F}, 0.08F, 0.12F);

    CHECK(hint(0, 0, 0) > 0.0F);
    CHECK(hint(0, 0, 0) < 1.0F);
}
